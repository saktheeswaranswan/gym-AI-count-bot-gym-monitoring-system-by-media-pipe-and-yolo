...
# For central horizontal lines (zone-based event count)
...
event_count = 0        # counts events based on right thumb crossing zone
was_in_yellow = False  # flag to track thumb zone state
...
# For reaction force computation on the right ankle (from pose)
prev_time_ankle = None
prev_ankle_pos = None
prev_velocity_ankle = None

# For moving box event: box moves automatically from left to right.
box_x = 100            # initial x position of the box
box_y = 100            # fixed y position of the box (adjust as needed)
box_w, box_h = 200, 200  # width and height of the box
box_speed = 5          # horizontal speed (pixels per frame)
box_event_count = 0    # counts events when right-hand palm enters the box
was_in_box = False     # flag to prevent multiple counts per entry

# ----------------- Setup Video Capture & Writer -----------------

cap = cv2.VideoCapture('power.mp4')  # live webcam input
...
while cap.isOpened():
    ...
    # Process frame with Holistic (for pose and hand landmarks)
    ...
    # Check for right-hand palm entering the box.
    # Estimate the palm center by averaging landmarks 0 (wrist), 5 (index MCP), and 17 (pinky MCP)
    if results.right_hand_landmarks:
    ...
    if results.right_hand_landmarks:
        ...
        if prev_finger_pos is not None:
            ...
            if prev_velocity_finger is not None:
                ...
                # Compute an orthogonal vector (90° rotation)
                ...
                # Resultant vector example: sum of normalized velocity and acceleration
